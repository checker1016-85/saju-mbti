...
function calcRadar(tg,r){
  const str=r.advanced?.dayStrength?.score||50;const clamp=(v)=>Math.round(Math.max(5,Math.min(100,v))*10)/10;const C='#3D6BA3';
  return {
    '체력·멘탈':{color:C,axes:[{label:'체력',value:clamp(tgVal(tg,['비견','겁재'])*16+str*.3)},{label:'멘탈력',value:clamp(tgVal(tg,['편인','정인'])*16+str*.2)},{label:'회복력',value:clamp(tgVal(tg,['식신','상관'])*14+tgVal(tg,['정인'])*8)},{label:'인내력',value:clamp(tgVal(tg,['편관','정관'])*12+tgVal(tg,['정재'])*10)},{label:'스트레스\n내성',value:clamp(str*.5+tgVal(tg,['비견'])*12)}]},
    '의지·결단':{color:C,axes:[{label:'결단력',value:clamp(tgVal(tg,['편관'])*16+tgVal(tg,['겁재'])*10)},{label:'독립심',value:clamp(tgVal(tg,['비견'])*16+tgVal(tg,['겁재'])*10+tgVal(tg,['편인'])*6)},{label:'인내심',value:clamp(tgVal(tg,['정관'])*14+tgVal(tg,['정재'])*12)},{label:'책임감',value:clamp(tgVal(tg,['정관'])*16+tgVal(tg,['정인'])*8)},{label:'의지력',value:clamp(tgVal(tg,['비견'])*14+tgVal(tg,['편관'])*10+str*.3)}]},
    '통찰·집중':{color:C,axes:[{label:'직관력',value:clamp(tgVal(tg,['편인'])*16+tgVal(tg,['상관'])*10)},{label:'통찰력',value:clamp(tgVal(tg,['편인'])*14+tgVal(tg,['정인'])*10+tgVal(tg,['편관'])*6)},{label:'집중력',value:clamp(tgVal(tg,['정인'])*16+tgVal(tg,['정관'])*10)},{label:'판단력',value:clamp(tgVal(tg,['정관'])*14+tgVal(tg,['편인'])*10)},{label:'관찰력',value:clamp(tgVal(tg,['편인'])*12+tgVal(tg,['식신'])*10+tgVal(tg,['정재'])*6)}]},
    '사회성':{color:C,axes:[{label:'리더십',value:clamp(tgVal(tg,['편관','정관'])*16)},{label:'소통력',value:clamp(tgVal(tg,['식신','상관'])*16)},{label:'협업력',value:clamp(tgVal(tg,['비견'])*18+tgVal(tg,['정관'])*8)},{label:'매력',value:clamp(tgVal(tg,['편재','정재'])*12+tgVal(tg,['상관'])*8)},{label:'공감력',value:clamp(tgVal(tg,['정인'])*14+tgVal(tg,['식신'])*10)}]},
    '감성·관계':{color:C,axes:[{label:'관계운',value:clamp(tgVal(tg,['정재'])*14+tgVal(tg,['식신'])*10+tgVal(tg,['정관'])*6)},{label:'배려심',value:clamp(tgVal(tg,['정인'])*14+tgVal(tg,['식신'])*10)},{label:'감수성',value:clamp(tgVal(tg,['상관'])*16+tgVal(tg,['정인'])*8)},{label:'표현력',value:clamp(tgVal(tg,['상관'])*14+tgVal(tg,['식신'])*12)},{label:'포용력',value:clamp(tgVal(tg,['비견'])*10+tgVal(tg,['정인'])*12+tgVal(tg,['식신'])*6)}]},
    '재능·두뇌':{color:C,axes:[{label:'창의력',value:clamp(tgVal(tg,['상관'])*18+tgVal(tg,['식신'])*10)},{label:'분석력',value:clamp(tgVal(tg,['정인'])*16+tgVal(tg,['편인'])*10)},{label:'실행력',value:clamp(tgVal(tg,['비견','겁재'])*12+tgVal(tg,['편관'])*10)},{label:'전략력',value:clamp(tgVal(tg,['정관'])*12+tgVal(tg,['정인'])*12)},{label:'직관력',value:clamp(tgVal(tg,['편인'])*16+tgVal(tg,['상관'])*10)}]},
    '재물·야망':{color:C,axes:[{label:'수익감각',value:clamp(tgVal(tg,['정재'])*18+tgVal(tg,['편재'])*8)},{label:'투자감각',value:clamp(tgVal(tg,['편재'])*18+tgVal(tg,['정재'])*6)},{label:'야망',value:clamp(tgVal(tg,['편관'])*16+tgVal(tg,['겁재'])*8)},{label:'안정추구',value:clamp(tgVal(tg,['정관'])*14+tgVal(tg,['정재'])*12)},{label:'모험도',value:clamp(tgVal(tg,['겁재'])*14+tgVal(tg,['편재'])*12)}]},
    '관계·연애':{color:C,axes:[{label:'애정표현',value:clamp(tgVal(tg,['정재','편재'])*14+tgVal(tg,['상관'])*8)},{label:'헌신도',value:clamp(tgVal(tg,['정재'])*16+tgVal(tg,['정인'])*8)},{label:'매력',value:clamp(tgVal(tg,['상관'])*14+tgVal(tg,['편재'])*10)},{label:'안정성',value:clamp(tgVal(tg,['정관','정재'])*12)},{label:'자유추구',value:clamp(tgVal(tg,['겁재','상관'])*12)}]},
    '직업·사회':{color:C,axes:[{label:'조직력',value:clamp(tgVal(tg,['정관'])*16+tgVal(tg,['정재'])*8)},{label:'전문성',value:clamp(tgVal(tg,['정인','편인'])*14)},{label:'추진력',value:clamp(tgVal(tg,['편관','겁재'])*14)},{label:'창업기질',value:clamp(tgVal(tg,['편재','상관'])*14)},{label:'협상력',value:clamp(tgVal(tg,['정재','식신'])*13)}]}
  };
}
