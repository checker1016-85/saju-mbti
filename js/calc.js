...
function calcRadar(tg,r){
  ...
  const C='#3D6BA3';
  return {
    '체력·멘탈':{color:C,axes:[{label:'체력',value:sc(['비견','겁재'],18,str*.3)},{label:'멘탈력',value:sc(['편인','정인'],18,str*.2)},{label:'회복력',value:sc(['식신','상관'],16,tgVal(tg,['정인'])*8)},{label:'인내력',value:sc(['편관','정관'],14,tgVal(tg,['정재'])*10)},{label:'스트레스\n내성',value:clamp(B+str*.5+tgVal(tg,['비견'])*14)}]},
    '의지·결단':{color:C,axes:[{label:'결단력',value:sc(['편관'],20,tgVal(tg,['겁재'])*12)},{label:'독립심',value:sc(['비견'],18,tgVal(tg,['겁재'])*12+tgVal(tg,['편인'])*6)},{label:'인내심',value:sc(['정관'],16,tgVal(tg,['정재'])*14)},{label:'책임감',value:sc(['정관'],18,tgVal(tg,['정인'])*10)},{label:'의지력',value:sc(['비견'],16,tgVal(tg,['편관'])*12+str*.3)}]},
    '통찰·집중':{color:C,axes:[{label:'직관력',value:sc(['편인'],20,tgVal(tg,['상관'])*12)},{label:'통찰력',value:sc(['편인'],16,tgVal(tg,['정인'])*12+tgVal(tg,['편관'])*6)},{label:'집중력',value:sc(['정인'],18,tgVal(tg,['정관'])*12)},{label:'판단력',value:sc(['정관'],16,tgVal(tg,['편인'])*12)},{label:'관찰력',value:sc(['편인'],14,tgVal(tg,['식신'])*12+tgVal(tg,['정재'])*6)}]},
    '사회성':{color:C,axes:[{label:'리더십',value:sc(['편관','정관'],18)},{label:'소통력',value:sc(['식신','상관'],18)},{label:'협업력',value:sc(['비견'],20,tgVal(tg,['정관'])*10)},{label:'매력',value:sc(['편재','정재'],14,tgVal(tg,['상관'])*10)},{label:'공감력',value:sc(['정인'],16,tgVal(tg,['식신'])*12)}]},
    '감성·관계':{color:C,axes:[{label:'관계운',value:sc(['정재'],16,tgVal(tg,['식신'])*12+tgVal(tg,['정관'])*6)},{label:'배려심',value:sc(['정인'],16,tgVal(tg,['식신'])*12)},{label:'감수성',value:sc(['상관'],20,tgVal(tg,['정인'])*10)},{label:'표현력',value:sc(['상관'],16,tgVal(tg,['식신'])*14)},{label:'포용력',value:sc(['비견'],12,tgVal(tg,['정인'])*14+tgVal(tg,['식신'])*6)}]},
    '재능·두뇌':{color:C,axes:[{label:'창의력',value:sc(['상관'],22,tgVal(tg,['식신'])*12)},{label:'분석력',value:sc(['정인'],18,tgVal(tg,['편인'])*12)},{label:'실행력',value:sc(['비견','겁재'],14,tgVal(tg,['편관'])*12)},{label:'전략력',value:sc(['정관'],14,tgVal(tg,['정인'])*14)},{label:'직관력',value:sc(['편인'],20,tgVal(tg,['상관'])*12)}]},
    '재물·야망':{color:C,axes:[{label:'수익감각',value:sc(['정재'],22,tgVal(tg,['편재'])*10)},{label:'투자감각',value:sc(['편재'],22,tgVal(tg,['정재'])*8)},{label:'야망',value:sc(['편관'],18,tgVal(tg,['겁재'])*10)},{label:'안정추구',value:sc(['정관'],16,tgVal(tg,['정재'])*14)},{label:'모험도',value:sc(['겁재'],16,tgVal(tg,['편재'])*14)}]},
    '관계·연애':{color:C,axes:[{label:'애정표현',value:sc(['정재','편재'],16,tgVal(tg,['상관'])*10)},{label:'헌신도',value:sc(['정재'],20,tgVal(tg,['정인'])*10)},{label:'매력',value:sc(['상관'],16,tgVal(tg,['편재'])*12)},{label:'안정성',value:sc(['정관','정재'],14)},{label:'자유추구',value:sc(['겁재','상관'],14)}]},
    '직업·사회':{color:C,axes:[{label:'조직력',value:sc(['정관'],18,tgVal(tg,['정재'])*10)},{label:'전문성',value:sc(['정인','편인'],16)},{label:'추진력',value:sc(['편관','겁재'],16)},{label:'창업기질',value:sc(['편재','상관'],16)},{label:'협상력',value:sc(['정재','식신'],15)}]}
  };
}
// ═══ MBTI ═══
